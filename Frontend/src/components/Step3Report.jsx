...
const Step3Report = ({ report }) => {
 ...
 const questionScoreData = questionWiseScore.map((item, index) => ({
  ...
  const downloadPDF = () =>{
    ...
    doc.text("AI Interview Perfoemance Report", pageWidth/2, currentY ,{
      align:"center",
    })
    currentY +=5
...
doc.text(`Correctness: ${correctness}/10`, margin + 10, currentY + 26)
    ...
    if(finalScore >=8){
      advice = "Excellent performance. Maintain confidence and structure. Continue refining Clarity and supporting answer with strong real-word examples."
    }else if(finalScore >=5){
      advice = "Good foundation. Focus on enhancing communication clarity and providing more specific examples to support your answers."
    }else{
      advice = "Significant improvement needed. Work on structuring your answers, improving confidence, and providing relevant details."
    }
...
alternateRowStyles:{
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-gray-50 to-green-50 px-4 sm:px-6 lg:px-10 py-8'>
      ...
      <div className='w-full mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
