...
<h1 className='font-semibold hidden md:block text-lg'>HireGenie-AI</h1>
        </div>
...
(!showCreditPopup)
  setShowUserPopup(false)
    ...
    {showCreditPopup && (
        <div className='absolute right-12.5 mt-3 w-64 bg-white shadow-xl border-gray-200 rounded p-5 z-50'>
    ...
    {showUserPopup && (
        <div className='absolute right-0 mt-3 w-48 bg-white shadow-xl border border-gray-200 rounded-xl p-4 z-50'>
            ...
            <button
            onClick={()=>nevigate("/history")}
            className='w-full text-left text-sm py-2 hover:text-black text-gray-600'>InterView History</button>
            <button 
            onClick={(handleLogout)}
            className='w-full test-left text-sm py-2 flex items-center gap-2 text-red-500'>
